...
Class Master extends DBConnection {
	...
	function delete_booking(){
		extract($_POST);
		$delete = $this->conn->query("DELETE FROM `book_order` where id = '{$id}'");
		$delete2 = $this->conn->query("DELETE FROM `booking_list` where order_id = '{$id}'");
		$delete3 = $this->conn->query("DELETE FROM `sales` where order_id = '{$id}'");
		if($this->capture_err())
			return $this->capture_err();
		if($delete){
	...
	function update_order_status(){
		...
		extract($_POST);
		$update = $this->conn->query("UPDATE `book_order` set `status` = '$status' where id = '{$id}' ");
		if($update){
			$resp['status'] ='success';
	...
	function pay_booking(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `book_order` set `paid` = '1' where id = '{$id}' ");
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," Booking payment status successfully updated.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
